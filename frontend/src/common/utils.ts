...
export function extractStandardJavSn(sn: string): JavSn {
  const result: JavSn = { isJavSn: false };
...
export function getDmmThumbURL(sn: string, cover?: string) {
  if (cover) {
    return cover;
  }
...
function onNoCover(sn: string): string {
